...
void drawAllMonthlyHexagons() {
  ...
  for (int col = 0; col < 6; col++) {
    for (int row = 0; row < 2; row++) {
      ...
      float cy = 200 + row * 525; // Adjusted vertical spacing between rows for stacked hexagons
...
void drawMonthlyHexagons(String monthPrefix, float cx, float cy) {
  ...
  drawHexagonWithBorder(cx, cy + 275, getSeason(monthPrefix), "average", averageLow, averageHigh); // Average Low and High
}
