...
float hexRadius = 120; // Reduced radius of hexagon (distance from center to any corner)
float borderThickness = 15; // Adjusted thickness of the border hexagon

JSONObject temperatureData;

void setup() {
  size(2400, 2700); // Canvas size for 24 hexagons (6 columns x 4 rows)
...
void drawAllMonthlyHexagons() {
  ...
  for (int col = 0; col < 6; col++) {
    for (int row = 0; row < 2; row++) {
      ...
      float cx = 200 + col * 400; // Adjusted horizontal spacing between columns
      float cy = 200 + row * 600; // Adjusted vertical spacing between rows for stacked hexagons
...
void drawMonthlyHexagons(String monthPrefix, float cx, float cy) {
  ...
  drawHexagonWithBorder(cx, cy + 300, getSeason(monthPrefix), "average", averageLow, averageHigh); // Average Low and High
}
