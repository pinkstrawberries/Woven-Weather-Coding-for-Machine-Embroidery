...
final int rows = 26; // Number of hexagons per row (representing a fortnight)
final int columns = 14; // Number of hexagons per column (14 fortnights)

// Set up a larger canvas size to fit all hexagons properly
final int canvasWidth = (int)(rows * (1.5 * hexSize)) + 100; // Calculate width dynamically based on rows
final int canvasHeight = (int)(columns * (sqrt(3) * hexSize)) + 100; // Calculate height dynamically based on columns
